...
Item {
    ...
    // Charge level drives the color: green when healthy, red when critical
    function batteryColor() {
        if (charging)
            return IconPalette.batteryCharging;
        if (percent >= 80)
            return IconPalette.batteryFull;
        if (percent >= 50)
            return IconPalette.batteryGood;
        if (percent >= 30)
            return IconPalette.batteryMid;
        if (percent >= 15)
            return IconPalette.batteryLow;
        return IconPalette.batteryCritical;
    }
    ...
    PillIndicator {
        ...
        iconCircleColor: batteryWidget.batteryColor()
        iconTextColor: Theme.backgroundPrimary
        textColor: batteryWidget.batteryColor()
        collapsedIconColor: batteryWidget.batteryColor()
